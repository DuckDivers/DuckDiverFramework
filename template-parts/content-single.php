...
if ( 'large' === $featured_size ) {
	$featured_class = 'col-12';
} elseif ( 'small' === $featured_size ) {
	$featured_class = 'col-md-6 offset-md-3';
} else {
	$featured_class = 'd-none';}
$post_class = ( 'small' !== $featured_size ) ? 'col-12' : 'col-md-8';
?>

<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<?php
	if ( has_post_thumbnail() ) {
		echo '<div class="row"><figure class="featured-thumbnail thumbnail ' . esc_attr( $featured_class ) . '">' . get_the_post_thumbnail( $post->ID, 'blog-hero-image', array( 'class' => 'd-block mx-auto' ) ) . '</figure></div>';
	}
	?>
