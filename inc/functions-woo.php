 ...
 * @since Duck Diver Framework 1.1
 *
 */

if ( ! function_exists( 'woocommerce_support' ) ) {
	/**
	 * Adds theme support for various WooCommerce features including product gallery functionalities.
	 *
	 * @return void
	 */
...
if ( ! function_exists( 'dd_open_shop_content_wrappers' ) ) {
	function dd_open_shop_content_wrappers() {
		...
		if ( ( true === get_theme_mod( 'dd_theme_disable_archive_sidebar' ) && ! is_product() ) || ( true === get_theme_mod( 'dd_theme_disable_product_sidebar' ) && is_product() ) ) {
			...
			echo '<main id="main" class="' . esc_attr( $sbpos['main'] ) . '" role="main">';
		}
	}
}
...
if ( ! function_exists( 'dd_close_shop_content_wrappers' ) ) {
	/**
	 * Closes custom content wrappers on WooCommerce shop pages and optionally displays the sidebar.
	 *
	 * This function outputs the necessary HTML to close the main content area and, depending on the theme and page settings,
	 * renders the shop sidebar. The sidebar position is dynamically determined based on the theme's sidebar settings.
	 *
	 * @return void
	 */
	function dd_close_shop_content_wrappers() {
		$sbpos = dd_get_sidebar_position( 'wc_sidebar_position' );
			echo '</main>';
		if ( ! get_theme_mod( 'dd_theme_disable_archive_sidebar' ) && ! is_product() || ! get_theme_mod( 'dd_theme_disable_product_sidebar' ) && is_product() ) {
			echo '<aside class="' . esc_attr( $sbpos['sb'] ) . ' sidebar" id="sidebar">';
...
if ( ! function_exists( 'dd_prepare_shop_wrappers' ) ) {
	/**
	 * Prepares custom wrappers for the WooCommerce shop pages by modifying actions related to content and sidebar.
	 *
	 * This function removes default WooCommerce actions for content wrappers and sidebar,
	 * and replaces them with custom actions for better integration with a Bootstrap-based theme.
	 *
	 * @return void
	 */
